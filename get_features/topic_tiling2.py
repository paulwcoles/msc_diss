...
def lda_train_test(n_tiling_iterations, lda):
    ...
    for i in tiling_range:     # Store topic assignment per word, then retrain
        ...
        for doc_name, utt_matrices in word_wise_test_dataset.iteritems():
            for utt_index, utt_matrix in enumerate(utt_matrices):
                ...
                for word_index, word in enumerate(utt_lda):
                    test_corpus_topic_counts[doc_name][utt_index][word_index].append(np.argmax(word))
...
if __name__ == "__main__":
    ...
    if switch == 'doc2vec':
        ...
        lda_train_test(n_tiling_iterations, lda)
    print "-" * 30 + "  End  " + "-" * 30
